...
normative:
  ...
  OpenID.IPSIE-Common-Requirements:
    ...
    target: https://deansaxe.github.io/draft-saxe-ipsie-common-requirements-profile/draft-saxe-ipsie-common-requirements-profile.html
    date: July 9, 2025
    author:
      - ins: D. H. Saxe
  NIST.FAL:
...
Applications and Identity Providers MUST follow all of the requirements in the IPSIE Common Requirements Profile [OpenID.IPSIE-Common-Requirements].
...
* MUST support the `max_age` parameter with a value representing the maximum number of seconds allowable since the user last interactively authenticated at the OP. If the elapsed time since interactive authentication is less than this value, the OP MAY choose to actively reauthenticate the user.  If the elapsed time since interactive authentication is greater than this value, the OP MUST actively reauthenticate the user.
